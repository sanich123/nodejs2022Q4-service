...
export function checkPropertiesInTrackDto(trackDto: CreateTrackDto): boolean {
  ...
  for (const key in trackDto) {
    if (trackDto[key]) {
      ...
      const wrongTypeDuration = key === 'duration' && typeof trackDto[key] !== 'number';
      const wrongTypeArtistAlbumId = (key === 'albumId' || key === 'artistId') && typeof trackDto[key] !== 'string';
      if (wrongTypeName || wrongTypeDuration || wrongTypeArtistAlbumId) {
        falsyValues = true;
        break;
      }
    } else {
      if ((key === 'albumId' || key === 'artistId') && typeof trackDto[key] !== 'object') {
        falsyValues = true;
        break;
      }
    }
