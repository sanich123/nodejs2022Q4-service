...
describe('Users (e2e)', () => {
  ...
  describe('GET', () => {
    ...
    it('should correctly get user by id', async () => {
      ...
      const searchResponse = await unauthorizedRequest.get(usersRoutes.getById(id)).set(commonHeaders);

      expect(searchResponse.statusCode).toBe(StatusCodes.OK);
      expect(searchResponse.body).toBeInstanceOf(Object);

      const cleanupResponse = await unauthorizedRequest.delete(usersRoutes.delete(id)).set(commonHeaders);

      expect(cleanupResponse.statusCode).toBe(StatusCodes.NO_CONTENT);
    });

    it('should respond with BAD_REQUEST status code in case of invalid id', async () => {
      const response = await unauthorizedRequest.get(usersRoutes.getById('some-invalid-id')).set(commonHeaders);

      expect(response.status).toBe(StatusCodes.BAD_REQUEST);
    });

    it("should respond with NOT_FOUND status code in case if user doesn't exist", async () => {
      const response = await unauthorizedRequest.get(usersRoutes.getById(randomUUID)).set(commonHeaders);
  ...
  describe('POST', () => {
    it('should correctly create user', async () => {
      const response = await unauthorizedRequest.post(usersRoutes.create).set(commonHeaders).send(createUserDto);
    ...
    it('should respond with BAD_REQUEST in case of invalid required data', async () => {
      const responses = await Promise.all([
        ...
        unauthorizedRequest.post(usersRoutes.create).set(commonHeaders).send({ login: 'TEST_LOGIN' }),
        unauthorizedRequest.post(usersRoutes.create).set(commonHeaders).send({ password: 'TEST_PASSWORD' }),
        unauthorizedRequest.post(usersRoutes.create).set(commonHeaders).send({ login: null, password: 12345 }),
      ]);
  ...
  describe('PUT', () => {
    it('should correctly update user password match', async () => {
      ...
      const updateResponse = await unauthorizedRequest.put(usersRoutes.update(createdId)).set(commonHeaders).send({
      ...
      const updateResponse2 = await unauthorizedRequest.put(usersRoutes.update(createdId)).set(commonHeaders).send({
        oldPassword: createUserDto.password,
        newPassword: 'NEW_PASSWORD',
      });
      ...
      const cleanupResponse = await unauthorizedRequest.delete(usersRoutes.delete(createdId)).set(commonHeaders);

      expect(cleanupResponse.statusCode).toBe(StatusCodes.NO_CONTENT);
    });

    it('should respond with BAD_REQUEST status code in case of invalid id', async () => {
      const response = await unauthorizedRequest.put(usersRoutes.update('some-invalid-id')).set(commonHeaders).send({
        oldPassword: 'test',
        newPassword: 'fake',
      });
    ...
    it('should respond with BAD_REQUEST status code in case of invalid dto', async () => {
      const response = await unauthorizedRequest.put(usersRoutes.update(randomUUID)).set(commonHeaders).send({});
  ...
  describe('DELETE', () => {
    ...
    it('should respond with BAD_REQUEST status code in case of invalid id', async () => {
      const response = await unauthorizedRequest.delete(usersRoutes.delete('some-invalid-id')).set(commonHeaders);

      expect(response.status).toBe(StatusCodes.BAD_REQUEST);
    });

    it("should respond with NOT_FOUND status code in case if user doesn't exist", async () => {
      const response = await unauthorizedRequest.delete(usersRoutes.delete(randomUUID)).set(commonHeaders);
